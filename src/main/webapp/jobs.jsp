...
<link rel="stylesheet"
	href="css/owlcarousel/owl.theme.default.min.css">
<!-- main css -->
...
<body>
	...
	<div class="container-fluid fluid-nav">
		<div class="container cnt-tnar">
			<nav class="navbar navbar-expand-lg navbar-light bg-light tjnav-bar">
				...
				<a href="${pageContext.request.contextPath}/JobListServlet" class="nav-logo"> <img
					src="img/fast-logo.png">
				</a>
				...
				<div class="collapse navbar-collapse" id="navbarSupportedContent">
					<ul class="navbar-nav mr-auto my-2 my-lg-0 tnav-right tn-nav">
						<li class="nav-item dropdown"><a
							...
							aria-disabled="true">${cookie.email.value}</a>
							<div class="dropdown-menu tdropdown"
								aria-labelledby="navbarDropdown">
								<a class="dropdown-item" href="${pageContext.request.contextPath}/JobListServlet">Dashboard</a> 
								<c:if test="${user eq 'USER' }">
									...
									<a class="dropdown-item"
									href="${pageContext.request.contextPath}/CookieDeletionServlet">Đăng
									xuất</a>
							</div></li>
					</ul>
				</div>
    ...
    <ul class="nav nav-tabs search-nav-tabs" id="myTab" role="tablist">
        <li class="nav-item search-nav-item"><a class="nav-link" href="<c:url value="/"/>">TRANG CHỦ</a></li>
        <li class="nav-item search-nav-item"><a class="nav-link" href="<c:url value="/JobListServlet"/>">VIỆC LÀM</a></li>

        <c:choose>
            <c:when test="${cookie.role.value eq 'Employer'}">
                <li class="nav-item search-nav-item"><a class="nav-link" href="EmployerServlet">NHÀ TUYỂN DỤNG</a></li>
            </c:when>
            <c:when test="${cookie.role.value eq 'Admin'}">
                <li class="nav-item search-nav-item"><a class="nav-link" href="ManageAccountServlet">CÔNG CỤ QUẢN TRỊ</a></li>
            </c:when>
        </c:choose>
		<li class="nav-item search-nav-item"><a class="nav-link" href="https://www.youtube.com/" target="_blank">VỀ CHÚNG TÔI</a></li>
    </ul>
				...
				<div class="tab-content search-tab-content" id="myTabContent">
					...
					<div class="tab-pane stab-pane fade show active" id="home"
						...
						<form class="bn-search-form" action="SearchServlet" method="get">
							<div class="row">
					...
					<div class="job-board-wrap">
						...
						<c:choose>
							<c:when test="${not empty joblist }">
										...
										<div class="job pagi">
											<div class="job-content">
												...
												<div class="job-desc">
													<div class="job-title">
														<a
															href="<c:url value="/JobDetailServlet?job_id=${job.job_id }&username=${name}&user=USER"/>">${job.title}</a>
													</div>
													<div class="job-company">
														<a>${job.employer_name }</a> | <a class="job-address"><i
												...
												<div class="wrap-btn-appl">
													<form action="<c:url value="/ApplyServlet"/>" method="get">
														<input type="hidden" name="username" value="${name}" /> <input
															type="hidden" name="job_id" value="${job.job_id}" /> <input
								...
								<c:set var="RECORDS_PER_PAGE" value="${joblist}"></c:set>
	...
	<footer class="container-fluid copyright-wrap">
		<div class="container copyright">
			<p class="copyright-content">
				Copyright © 2023 <a href="#"> FastJob</a>. All Right Reserved.
			</p>
		</div>
	</footer>
